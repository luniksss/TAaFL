# Лексическая структура языка программирования МЯУ#
## 1. Примеры кода
```
лапкапомощи()
{
  мур цыфорка число1 мяу
  мур цыфорка число2 мяу

  клацать(число1) мяу
  клацать(число2) мяу

  мур цыфорка сумма мяу
  сумма царапнуть сложитьЧиселки(число1, число2) мяу
  мурлыкать(сумма) мяу
}

лапка сложитьЧиселки(цыфорка слагаемое1, цыфорка слагаемое2): цыфорка
{
  вернуть слагаемое1 + слагаемое2 мяу
}
```

## 2. Ключевые слова
ключевые слова _регистронезависимые_

| Слово | Предназначение                          |
| ----- | --------------------------------------- |
| мур | объявление переменной (как let) |
|строгиймур| объявление константы (const) |
|мурлыкать| вывод (output)|
|клацать|ввод (input)|  
|лапка| объявление функции (function)|
|лапкапомощи| объявление главной функции (main())|
|вернуть| возвращение из функции (return)|
|мягколапка| функция для преобразования целочисленного значения в действительное|  
|твердолапка| функция для преобразования действительного значения в целочисленное|  
  
Таблица возможных типов данных:  
| Слово | Тип                          |
| ----- | --------------------------------------- |
|цыфорка|целочисленное значение (int) |
|рилцыфорка| действительное число (float)|
|кринжли| bool|
|кринж| bool true|
|некринж| bool false|  
  
_Функция может возвращать 4 типа: цыфорка, рилцыфорка, кринжли и ничего(аналог void)_

Ветвления и циклы:
| Слово | Предназначение                          |
| ----- | --------------------------------------- |
| триппитроппа | условие (if) |
| троппатриппа | противовес условию (else) |
| магасияй | цикл(while) |
| хотябыраз магасияй | цикл (do while) |
| хвостомкрутить | цикл(for) |
| стоп | остановиться и выйти из цикла (break) |

## 3. Идентификаторы
Идентификатор может:
1) Начинаться с буквы или нижнего подчёркивания "_"
2) Содержать буквы, арабские цифры 0, 1, ..., 9 и нижние подчёркивания "_"

## 4. Литералы
### 4.1. Литералы чисел
Литерал целого числа, состоящий из арабских цифр 0, 1, ..., 9
Действительные числа разделяются точкой

## 5. Операторы
Язык поддерживает следующие операторы:  
- сложение (и унарный "+") - `+`  
- вычитание (и унарный "-") - `-`
- возведение в степень - `^`   
- умножение - `*`  
- деление нацело - `/`
- остаток от деления - `%`
- строгое больше - `>`  
- нестрогое больше - `>=`  
- строгое меньше - `<`  
- нестрогое меньше - `<=`
- нестрогое равенство - `==`
- неравенство - `!=`
- логическое И - `&&`
- логическое ИЛИ - `||`
- логическое НЕ - `!`

## 6. Прочие лексемы
Список прочих лексем:
- `мяу` вместо `;` — используется как разделитель команд
- `царапнуть` вместо `=` - используется при присвоении значения  
- `,` — используется как разделитель в параметрах функций
- `(` и `)` - используются в функиях для прописывания параметров

## 7. Комментарии
ЯП допускает только однострочные комментарии, которые начинаются через пробел со слова `окак`.   
например: `окак в функции временный костыль`
